package cz.osu.ratingservice.error.exceptionHandler;

import cz.osu.ratingservice.error.exception.*;
import cz.osu.ratingservice.model.response.MessageResponse;
...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    @ExceptionHandler({IllegalStateException.class, AlreadyRatedException.class, RatingCreationException.class, RatingDeletingException.class,
            RatingNotFoundException.class, RatingOutOfBoundsException.class, RatingUpdatingException.class, ReviewNotFoundException.class,
            ConstraintViolationException.class,RetryFallException.class})
    ...
    @ExceptionHandler({RatingNotAvailableException.class})
    @ResponseStatus(value = HttpStatus.TOO_EARLY)
    public MessageResponse TooEarlyErrorException(Exception exception, WebRequest request) {
        log.error(String.format("Invoked exception type: TOO_EARLY, full message: %s ", exception.getMessage()));

        return new MessageResponse(HttpStatus.TOO_EARLY, exception.getMessage(), Instant.now().toString());
    }
